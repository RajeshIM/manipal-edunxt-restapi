...
describe('Learner LeaderBoard API', function () {
  describe('/api/v1/lnd/learner-leaderboard', function () {
    ...
    //   chai.request(server)
    //     .post('/api/v1/lnd/learner-leaderboard')
    //     .end(function (err, res) {
      ...
      chai.request(server)
        ...
        .end(function (err, res) {
          ...
          if (res.body.data.length > 0) {
            expect(res.body.data[0]).to.have.all.keys('learnerId', 'learnerName', 'learnerSerialNumber', 'actionMessage', 
                                                      'pointsEarned','pointsEarnedSinceLastMonth','testPerformance',
            ...
            expect(res.body.data[0].learnerSerialNumber).to.be.a('string');
            expect(res.body.data[0].actionMessage).to.be.a('string');
            expect(res.body.data[0].pointsEarned).to.be.a('string');
            expect(res.body.data[0].pointsEarnedSinceLastMonth).to.be.a('string');
            expect(res.body.data[0].testPerformance).to.be.a('number');
            expect(res.body.data[0].avgTestPerformance).to.be.a('number');
            expect(res.body.data[0].examScore).to.be.a('number');
          }
          done();
        })
    });
    ...
    it('should give in between days data in response if valid LnDUserId is passed', function (done) {
      ...
      chai.request(server)
        .post('/api/v1/lnd/learner-leaderboard?start_date=' + startDate + '&end_date=' + endDate)
        .set({
          'LnDUserId': 1
        })
        .end(function (err, res) {

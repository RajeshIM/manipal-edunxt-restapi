...
describe('Organization Interests API', function () {
  describe('/api/v1/lnd/organization-interests', function () {
    ...
    //   chai.request(server)
    //     .post('/api/v1/lnd/organization-interests')
    //     .end(function (err, res) {
    ...
    it('should give the default response if invalid LnDUserId is passed', function (done) {
      ...
      chai.request(server)
        ...
        .end(function (err, res) {
          ...
          expect(res.body.data).to.have.all.keys('topInterestsData', 'popularTopicsData');
          expect(res.body.data.topInterestsData).to.be.an('array').that.is.empty;
          expect(res.body.data.popularTopicsData).to.be.an('array').that.is.empty;
          done();
        })
    });
    ...
    it('should give the success response if valid LnDUserId is passed', function (done) {
      ...
      chai.request(server)
        ...
        .end(function (err, res) {
          ...
          expect(res.body.data).to.be.a('object');
          expect(res.body.data).to.have.all.keys('topInterestsData', 'popularTopicsData')
          if (res.body.data.topInterestsData > 0) {
          ...
          if (res.body.data.popularTopicsData > 0) {
            expect(res.body.data[0]).to.have.all.keys('courseId', 'courseName', 'numberOfLikes');
            expect(res.body.data[0].courseId).to.be.a('number');
            expect(res.body.data[0].courseName).to.be.a('string');
            expect(res.body.data[0].numberOfLikes).to.be.a('string');
          }
          done();
        })
    });
    ...
    it('should give in between days data in response if valid LnDUserId is passed', function (done) {
      ...
      chai.request(server)
        .post('/api/v1/lnd/organization-interests?start_date=' + startDate + '&end_date=' + endDate)
        .set({
          'LnDUserId': 1
        })
        .end(function (err, res) {
